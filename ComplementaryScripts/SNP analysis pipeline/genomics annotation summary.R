...
mRNA <- s288_mRNA[index1]
mRNA <- str_replace_all(mRNA, ">","")
mRNA <- str_replace_all(mRNA, " \\[","@")
mRNA <- str_replace_all(mRNA, "\\]","")
mRNA0 <- str_split(mRNA,"@")
...
for (i in seq(length(mRNA0))){
  locus[i] <- which(str_detect(mRNA0[[i]],"locus_tag="))
  gene[i] <- mRNA0[[i]][locus[i]]
  location_index[i] <- which(str_detect(mRNA0[[i]],"location="))
  location[i] <- mRNA0[[i]][location_index[i]]
}

mRNA_fna <- data.frame(gene=gene, location= location, stringsAsFactors = FALSE)


seq_mRNA <- list()
for (i in seq(length(index1)-1)){
  seq_mRNA[[i]] <- s288_mRNA[(index1[i]+1):(index1[i+1]-1)]
}
seq_mRNA[[6301]] <- s288_mRNA[(index1[length(index1)]+1):length(s288_mRNA)]

nchar(paste(seq_mRNA[[6301]], sep = "", collapse = ""))

for (i in 1:6301){
  mRNA_fna$mRNA[i] <- paste(seq_mRNA[[i]], sep = "", collapse = "")
  mRNA_fna$length_mRNA[i] <- nchar(mRNA_fna$mRNA[i])
}

mRNA_fna$gene <- str_replace_all(mRNA_fna$gene, "locus_tag=", "")
