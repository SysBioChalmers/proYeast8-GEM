...
strain_glycerol <- read.table("data/strain_glycerol_classification.txt", header = TRUE, stringsAsFactors = FALSE)
strain_classification <- read_excel("data/strain_classification.xls")
strain_select1 <- strain_classification[which(strain_classification$Standardized_name %in% strain_glycerol$strain_name ==FALSE),]




# choose the interested protein and the related mutation datasets
gene_mutation  <- read.table("result/gene_mutation_related_to_glycerol.txt", header = TRUE, stringsAsFactors = FALSE)
...
cluster1$type <- 'a. two or three mutations'

strain2 <- mutation_matrix0[mutation_matrix0$sum >= 1 & mutation_matrix0$sum <= 1,] %>% select(.,strain_name)
cluster2 <-  growth_phenotype[growth_phenotype$strain_name %in% strain2$strain_name,]
cluster2$type <- 'b. one mutation'
