...
gene_mutation  <- read.table("result/gene_mutation_related_to_growth.txt", header = TRUE, stringsAsFactors = FALSE)
...
#here we produce the mutation list without considering the mutation result, but the position
mutation_summary1 <- mutation_summary
mutation_summary1$mutation_result <- str_replace_all(mutation_summary1$mutation_result,"YJL052W@@[:alpha:]", "")
colnames0 <- colnames(mutation_summary1)
colnames0 <- str_replace_all(colnames0, "YJL052W@@[:alpha:]", "")
colnames(mutation_summary1) <- colnames0
gene_mutation1 <- str_replace_all(gene_mutation1, "YJL052W@@[:alpha:]", "")
...
for(i in gene_mutation1){
  print(i)
  checkExist(i)
  mutation_summary[,i] <- checkExist(i, mutation_list=mutation_summary1)
}
...
strain1 <- mutation_matrix0[mutation_matrix0$sum >= 2,] %>% select(.,strain_name)
cluster1 <-  growth_phenotype[growth_phenotype$strain_name %in% strain1$strain_name,]
cluster1$type <- 'g1'

strain2 <- mutation_matrix0[mutation_matrix0$sum >= 0 & mutation_matrix0$sum <= 1,] %>% select(.,strain_name)
cluster2 <-  growth_phenotype[growth_phenotype$strain_name %in% strain2$strain_name,]
cluster2$type <- 'g2'

strain3 <- mutation_matrix0[mutation_matrix0$sum <=2,] %>% select(.,strain_name)
cluster3 <-  growth_phenotype[growth_phenotype$strain_name %in% strain3$strain_name,]
cluster3$type <- 'g3'   #'SACE_YDK' not in growth_phenotype
...
# method1
p <- ggplot(strain_classification0, aes(x=type,y=growth, fill=type))
...
# Add data points
strain_classification0$type <- as.factor(strain_classification0$type)
mylevels<-levels(strain_classification0$type)
levelProportions<-summary(strain_classification0$type)/nrow(strain_classification0)

for(i in 1:length(mylevels)){
  thislevel<-mylevels[i]
  thisvalues<- unlist(strain_classification0[strain_classification0$type==thislevel, "growth"],use.names = FALSE)
